...
<form class="mt-3" method="POST" enctype="multipart/form-data" action="{{ route('portal.pets.update', $pet) }}">
  ...
  <div class="row g-3">
    ...
    <div class="col-md-12">
      ...
      <textarea name="medical_conditions" class="form-control" rows="3">{{ old('medical_conditions', $pet->medical_conditions) }}</textarea>
    </div>
    <div class="col-md-6">
      <label class="form-label">Foto</label>
      <input type="file" name="photo" class="form-control">
      @if($pet->photo)
        <img src="{{ asset('storage/'.$pet->photo) }}" class="img-fluid mt-2 rounded" style="max-height:160px">
      @endif
    </div>
  </div>
